...
function  getPerfList(flag){
	...
	$.ajax({
		type:"post",
		url:"/api/mms-api/performanceTransferCheck/search",
		async:true,
		contentType:'application/x-www-form-urlencoded',	
		data:param,
		success:function(data){
...
function details(id,status){
	...
	$.ajax({
		type:'get',
		url:'/api/mms-api//performanceTransferCheck/'+ id +'/details/',
		async:true,
		success:function(data){
			 if((data.statusCode == 200) && (data.data.length != 0)){
			 	...
			 	$.each(data.data, function(index,item) {
			 		html += '<tr>';
						html += '<td>'+ item.createTime +'</td>';
						html += '<td>'+ item.operatorName +'</td>';
						html += '<td>'+ item.originalSales.orgName +'</td>';
						html += '<td>'+ item.originalSales.salesName +'</td>';
						html += '<td>'+ item.originalSales.salesMobile +'</td>';
						html += '<td>'+ item.receivedSales.orgName +'</td>';
						html += '<td>'+ item.receivedSales.salesName +'</td>';
						html += '<td>'+ item.receivedSales.salesMobile +'</td>';
...
function resolve(){	
	$.ajax({
		type:'post',
		url:'/api/mms-api/performanceTransferCheck/'+ nowId +'/pass',
		async:true,
		success:function(data){
			$('.modal').modal('hide');
			getPerfList();
...
function reject(){
	$.ajax({
		type:'post',
		url:'/api/mms-api/performanceTransferCheck/'+ nowId +'/refuse',
		async:true,
		success:function(data){
			$('.modal').modal('hide');
			getPerfList();
