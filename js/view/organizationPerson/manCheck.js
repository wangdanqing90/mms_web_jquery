...
function getCheckTypes(){
	$.ajax({
		type:"get",
		url:"/api/mms-api/salesTransferCheck/getCheckTypes",		
			...
			if(data.statusCode == 200) {
				if(data.data.checkTypeList.length > 0){
					...
					$.each(data.data.checkTypeList,function(index,item){
						html += '<option value='+item.code+' class="span6">'+item.desc+'</option>';
					})
					$('.s-type').append(html);
				}
...
function  getManageList(flag){	
	...
	$.ajax({
		type:"post",
		url:"/api/mms-api/salesTransferCheck/search",		
		contentType:'application/x-www-form-urlencoded',
		async:true,
		data:param,
		success:function(data){
...
function details(id,status){
	...
	$.ajax({
		type:'get',
		url:'/api/mms-api/salesTransferCheck/'+ id +'/details',
		async:true,
		success:function(data){
			 if((data.statusCode == 200) && (data.data.length != 0)){
			 	var html = "";
			 	$.each(data.data, function(index,item) {
			 		item.originalOrgName = item.originalOrgName == null ? " " : item.originalOrgName;
			 		item.targetOrgName = item.targetOrgName == null ? " " : item.targetOrgName;
			 		item.salesPhone = item.salesPhone == null ? " " : item.salesPhone;
			 		html += '<tr>';
						html += '<td>'+ item.createTime +'</td>';
						html += '<td>'+ item.operatorName +'</td>';
						html += '<td>'+ item.originalOrgName +'</td>';
						//html += '<td>'+ item.salesPhone +'</td>';						
						html += '<td>'+ item.targetOrgName +'</td>';
...
function resolve(){	
	$.ajax({
		type:'post',
		url:'/api/mms-api/salesTransferCheck/'+ nowId +'/pass',
		async:true,
		success:function(data){
			$('.modal').modal('hide');
			getManageList();
...
function reject(){
	$.ajax({
		type:'post',
		url:'/api/mms-api/salesTransferCheck/'+ nowId +'/refuse',
		async:true,
		success:function(data){
			$('.modal').modal('hide');
			getManageList();
